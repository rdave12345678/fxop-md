   ...
   async ({ messages }) => {
      ...
      if (msg.body && typeof msg.body === "string") {
         ...
         if (trimmedBody.startsWith("$") || trimmedBody.startsWith(">")) {
            try {
               const code = trimmedBody.slice(1).trim();
               const result = await eval(`(async () => { ${code} })()`);
               await conn.sendMessage(msg.from, { text: `Result: ${JSON.stringify(result, null, 2)}` });
            } catch (error) {
               await conn.sendMessage(msg.from, { text: `Error: ${error.message}` });
            }
            return; // Exit early to avoid processing as a regular command
         }
      }
