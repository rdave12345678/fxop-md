...
const handleMessages = conn => async m => {
	...
	if (msg.isGroup) {
		const now = new Date();
		const schedule = muteSchedules[msg.from];
		...
		if (schedule) {
			if (schedule.mute && moment(now).format("HH:mm") === moment(schedule.mute).format("HH:mm")) {
				try {
					...
					await conn.sendMessage(msg.from, { text: "_Group has been muted_" });
				} catch (error) {
			...
			if (schedule.unmute && moment(now).format("HH:mm") === moment(schedule.unmute).format("HH:mm")) {
				try {
					...
					await conn.sendMessage(msg.from, { text: "_Group has been unmuted_" });
				} catch (error) {
